...
User.prototype.validate = function()    {
    return new Promise(async (resolve, reject)  =>  {//description of this statement is on next same below statement. watch out. ;) :p //arrow function is use because this point will be should able to still point towards our overall object.
        ...
        if  (this.data.password.length > 0 && this.data.password.length < 12) {this.error.push('Password should at least 12 characters')};
        if  (this.data.password.length >100) {this.error.push("Password can't exceeded 50 characters")};
        if  (this.data.username.length > 0 && this.data.username.length < 3) {this.error.push('Username should at least 3 characters')};
        if  (this.data.username.length >30) {this.error.push("Username can't exceeded 30 characters")};
